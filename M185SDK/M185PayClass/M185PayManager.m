    ...
    if ([config isKindOfClass:[M185PayConfig class]]) {
        ...
        if (SY185SDK) {
            ...
            if ([SY185SDK respondsToSelector:selector]) {
                func(SY185SDK,selector, config.serverID,config.serverName,config.roleID,config.roleName,config.productID,config.productName,config.amount,config.extension);
            }
        } else {
